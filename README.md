...
Распакуйте проект в любом удобном месте.

---

## Auth Service

Откройте терминал, перейдите в директорию `auth_service` и выполните:
...
После этого можно перейти по ссылке http://0.0.0.0:8000/docs#/ для доступа к Swagger UI.

---

## Bot Service

Откройте **новый терминал**, перейдите в директорию `bot_service` и выполните:
...
Убедитесь, что у вас локально запущен `redis-server`.
...
Теперь вам необходимо от `@BotFather` в Telegram зарегистрировать бота и получить API ключ к нему.

Полученный ключ необходимо вставить в файл `.env` в строку:
```
TELEGRAM_BOT_TOKEN=
```
после знака равенства, без кавычек.

После успешной конфигурации, можно запускать сервис. Для этого сначала запустите приложение Celery:
```
uv run celery -A app.infra.celery_app worker --loglevel=info
```

Теперь откройте **новый терминал**, так же перейдите в директорию `bot_service` и активируйте `.venv`, затем выполните:
```
uv run python -m app.bot.polling
```

После этого найдите бота в Telegram и начинайте пользоваться.
